...
const (
	...
	DatabaseService_ChangeSet_FullMethodName       = "/sql.v1.DatabaseService/ChangeSet"
)
...
type DatabaseServiceClient interface {
	...
	ChangeSet(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ChangeSetRequest, ChangeSetResponse], error)
}
...
func (c *databaseServiceClient) ChangeSet(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ChangeSetRequest, ChangeSetResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &DatabaseService_ServiceDesc.Streams[3], DatabaseService_ChangeSet_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ChangeSetRequest, ChangeSetResponse]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type DatabaseService_ChangeSetClient = grpc.BidiStreamingClient[ChangeSetRequest, ChangeSetResponse]
...
type DatabaseServiceServer interface {
	...
	ChangeSet(grpc.BidiStreamingServer[ChangeSetRequest, ChangeSetResponse]) error
...
func (UnimplementedDatabaseServiceServer) ChangeSet(grpc.BidiStreamingServer[ChangeSetRequest, ChangeSetResponse]) error {
	return status.Error(codes.Unimplemented, "method ChangeSet not implemented")
}
...
func _DatabaseService_ChangeSet_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(DatabaseServiceServer).ChangeSet(&grpc.GenericServerStream[ChangeSetRequest, ChangeSetResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type DatabaseService_ChangeSetServer = grpc.BidiStreamingServer[ChangeSetRequest, ChangeSetResponse]
...
var DatabaseService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ChangeSet",
			Handler:       _DatabaseService_ChangeSet_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
